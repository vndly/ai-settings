...
- Scope test runs as narrowly as the change allows — a single test, then its file or module; run the full suite only when I ask
...
- Use the structured user-input tool when available for decisions that are mine; otherwise ask directly: what's at stake, then concrete options, one marked as your recommendation, and always a free-text choice so I can answer in my own words
...
- Subagents are pre-authorized and preferred. Delegate with the collaboration or subagent tools whenever a task fits an available agent type. Never wait for me to ask.
- If something still blocks delegation, stop and ask whether to launch the subagent. Never silently run the task inline instead.

# Responses

Substantive replies — work reported, findings, a review or plan — use this shape. Acknowledgments, one-liners, mid-task check-ins, answers to my questions, inquiries, explanations, comparisons and structured user-input turns answer plainly instead.
